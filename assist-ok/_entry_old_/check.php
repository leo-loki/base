...
if((isset($_POST['zip1']))&&($_POST['zip1']!="")){ $address = $_POST['zip1']; }
if((isset($_POST['zip2']))&&($_POST['zip2']!="")){	$address .= "-".$_POST['zip2']; }
if((isset($_POST['addr']))&&($_POST['addr']!="")){	$address .= " ".$_POST['addr']; }
if((isset($_POST['course']))&&($_POST['course']!="")){ $course = $_POST['course']; }
if((isset($_POST['takuji']))&&($_POST['takuji']!="")){ $takuji = $_POST['takuji']; }
...
if((isset($_POST['checker']))&&($_POST['checker']!="")){
	$checkArray = $_POST['checker'];
	if((isset($_POST['checklimit']))&&($_POST['checklimit']!="")&&($_POST['checklimit']>0)){
	 $limit = $_POST['checklimit'];
	} else {
	 $limit = 5;
	}
	$favFlag = ""; $favHtml = "";
	 for($i=0; $i<$limit; $i++){
			if((isset($checkArray[$i]))&&($checkArray[$i]!="")){
				$favFlag .= $checkArray[$i]." ";
				$favHtml .= $checkArray[$i]."<br>";
			}
		}
	$msg .= "ご希望コース：".$favFlag."\r\n";
} else { $favFlag = false; }
...
<?php if($address){ ?><tr>
<th>ご住所</th>
<td><?=$address?><?php if($telFlag){ ?><br />ご連絡先：<?=$telFlag?><?php } ?></td>
</tr><?php } ?>
<?php if($course){ ?><tr>
<th>ご希望コース</th>
<td><?=$course?></td>
