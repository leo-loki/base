...
if(isset($langFlag)){
 ...
 $site_url = "https://i-studio.jp/"; // メールフッタに記載される運営者のWebサイトURL
$site_name = "i Creative Studio."; // 利用者への確認メール内で表示されるサイト名（通常は正式な企業名や店舗名を記述）
$send_name = "iCS"; // 確認メールの送信者名（メールヘッダ表記用・上記「サイト名」と同じでも可。なるべく少ない文字数で）
$send_mail = "hiro.iruy@gmail.com"; // 確認メールおよび運営者へのメール送信アドレス
 ...
 $back_url = $site_url."entry/"; // 送信後の画面遷移先
...
if(isset($_POST)){
 foreach ($_POST as $key => $value){
  ...
  if(is_array($value)){ $limit = count($value);
  } else { $limit = 1; }
  if($limit > 1){
   $post_method["$key"] = "";
...
<!-- CSS Loading //-->
<link rel="stylesheet" type="text/css" href="./css/style.css?id=mod20181005"><!-- form custom css -->
...
<?php
/*ここから・・・*/
if((isset($_GET['mode']))&&($_GET['mode']==="check")&&(isset($_POST['mode']))&&($_POST['mode']==="check")){
 include("check.php");
} else if((isset($_GET['mode']))&&($_GET['mode']==="send")&&(isset($_POST['mode']))&&($_POST['mode']==="send")){
